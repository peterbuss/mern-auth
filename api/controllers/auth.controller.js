    ...
    try {
        ...
        if(user) {
            ...
            const generatedPassword = Math.random().toString(36).slice(-8) +
                ...
                // 8 digits + 8 digits
            console.log("nu - 2, generated Password", generatedPassword);
            const hashedPassword = bcryptjs.hashSync(generatedPassword, 10);
            console.log("nu - 3");
            let n2 = req.body.name.split(" ").join("").toLowerCase() + Math.floor(Math.random() * 10000).toString() ;
...
export const signout = (req, res) => {
    res.clearCookie('access_token').status(200).json('Signout succesful!');
};
